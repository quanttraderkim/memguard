...
def _load_isb_module():
    ...
    spec = importlib.util.spec_from_file_location("memguard_isb_run", path)
...
def test_saturation_benchmark_exposes_overflow_at_50_and_500():
    ...
    row = next(item for item in results["protected_50"]["memguard"] if item["token_budget"] == 500)
...
def test_saturation_benchmark_keeps_20_rules_at_1000_for_memguard():
    module = _load_isb_module()
    results = module.run_saturation_benchmark()

    row = next(item for item in results["protected_20"]["memguard"] if item["token_budget"] == 1000)
